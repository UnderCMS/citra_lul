    ...
    if (backend->AllowsCachedReads()) {
        ...
        ctx.RunAsync(
            [this, async_data](Kernel::HLERequestContext& ctx) {
                ...
                const auto read_delay =
                    static_cast<s64>(backend->GetReadDelayNs(async_data->length));
                if (!async_data->cache_ready) {
                    const auto time_took =
                    ...
                    */
                    return static_cast<s64>((read_delay > time_took) ? (read_delay - time_took)
                                                                     : 0);
                } else {
        ...
        ResultVal<std::size_t> read = backend->Read(offset, length, *data);
        if (read.Failed()) {
            rb.Push(read.Code());
            rb.Push<u32>(0);
        } else {
