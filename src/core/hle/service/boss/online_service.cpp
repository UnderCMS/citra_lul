...
OnlineService::OnlineService(Core::System& system_) : system{system_} {
...
u16 OnlineService::GetNsDataIdList(const u32 filter, const u32 max_entries,
    ...
    for (const auto& current_entry : ns_data) {
        const u16 datatype_high =
            static_cast<u16>(static_cast<u32>(current_entry.header.datatype) >> 16);
        const u16 datatype_low =
            static_cast<u16>((static_cast<u32>(current_entry.header.datatype) & 0xFFFF));
